...
class TestChangeTodo:
    """Test the change todo endpoint."""

    def test_workspace_owner_change_todo(self, client: TestClient, login_user: Tuple[TestUserInfo, str], test_workspace_info: TestWorkspaceInfo, test_todolist_info: TestTodoListInfo) -> None:
        """Test that a workspace owner can change a todo in the workspace in the todolist."""
        ...
        response = client.put(
            ...
            json = {
                ...
                "workspace_default_name": test_workspace_info.workspace_default_name,
                "todolist_id": int(create_todolist_response.json()["data"]),
                "todo_id": int(create_todo_response.json()["data"]),
        ...
        with DatabaseConnection() as db:
            ...
            assert result.todo_id == int(create_todo_response.json()["data"])
            assert result.name == "new_testing"
            assert result.description == "new_testing"
            assert result.due_date == datetime.datetime(2021, 1, 2)
            assert result.status == "pending"
            assert result.priority == "low"
    
    def test_workspace_member_change_todo(self, client: TestClient, login_users: Tuple[Tuple[TestUserInfo, str], Tuple[TestUserInfo, str]], test_workspace_info: TestWorkspaceInfo, test_todolist_info: TestTodoListInfo) -> None:
        """Test that a workspace member can change a todo in the workspace in the todolist."""
